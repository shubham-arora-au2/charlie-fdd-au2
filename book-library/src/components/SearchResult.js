        ...
        return <div className="fluid-container">
            {this.props.booksList ?
                ...
                {this.props.booksList.map((item, i) =>
                    <li key={i.toString()} className="col-md-2" style={{backgroundColor:'lightgrey', margin:'10px'}}><Link to={'/read/' + (item.id.bookId || item.id)}>
                        {item.volumeInfo.imageLinks ? <img style={{marginTop:'10px'}} className="card-img-top" width="50px" height="235px" src={item.volumeInfo.imageLinks.thumbnail} alt='image...'/> : 
